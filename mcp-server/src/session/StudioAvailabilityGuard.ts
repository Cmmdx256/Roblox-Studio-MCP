...
import { studioSessionManager, SessionAvailabilityLevel } from './StudioSessionManager.js';
...
const CAPABILITY_REQUIREMENT_LEVELS: Record<RequiredCapability, SessionAvailabilityLevel> = {
...
const LEVEL_ORDER: Record<SessionAvailabilityLevel, number> = {
    NONE:               0,
    BRIDGE_ONLY:        1,
...
export class StudioAvailabilityGuard {
     ...
     */
    public async check(capability: RequiredCapability, operationDescription = 'Studio operation'): Promise<GuardResult> {
    ...
    private hasCapability(session: Readonly<any>, capability: RequiredCapability): boolean {
        switch (capability) {
    ...
    private getCapabilityHint(capability: RequiredCapability): string {
        switch (capability) {
            case 'INJECT_INPUT':
     ...
     */
    public async require(capability: RequiredCapability, operationDescription?: string): Promise<void> {
