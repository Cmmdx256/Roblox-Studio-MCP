...
export class TransactionEngine {
     ...
     */
    public async beginTransaction(name: string, description?: string): Promise<Transaction> {
        const id = uuidv4();
    ...
    public async executeStep(
    ...
    ): Promise<{ success: boolean; result: any; verification?: VerificationReport; error?: string }> {
        ...
        if (options.inverseAction) tx.manifest.rollbackActions.push(options.inverseAction);
        const target = String(params.path ?? params.parent ?? '');
        if (target) {
            if (action.includes('script')) tx.manifest.affectedScripts.push(target);
            else tx.manifest.affectedInstances.push(target);
        }
     ...
     */
    public async commitTransaction(transactionId: string): Promise<{ success: boolean; transaction: Transaction }> {
        const tx = this.transactions.get(transactionId);
     ...
     */
    public async rollbackTransaction(transactionId: string, reason?: string): Promise<{ success: boolean; rolledBackStepsCount: number }> {
        const tx = this.transactions.get(transactionId);
        if (!tx) throw new Error(`Transaction ${transactionId} not found`);
