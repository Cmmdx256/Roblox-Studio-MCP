...
import { StructuredIntent, StructuredRequirement } from './IntentEngine.js';
...
function mapCategoryToStage(category: string): { stage: number; opType: ChangeOperationType; riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' } {
    const stageMap: Record<string, { stage: number; opType: ChangeOperationType; riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' }> = {
...
function deriveTargetPath(req: StructuredRequirement, opType: ChangeOperationType): string {
...
function generateScaffoldSource(req: StructuredRequirement): string {
...
export class ChangePlanEngine {
     ...
     */
    public generatePlan(intent: StructuredIntent, criteriaSuite: AcceptanceSuite): StructuredChangePlan {
