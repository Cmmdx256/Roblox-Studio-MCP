import { UIScreenSpec, UIComponentSpec, UITheme, CompiledUIInstance } from '../ui/types.js';
...
export class UIDesignEngine {
     ...
     */
    public getTheme(themeId = 'dark_fantasy'): UITheme {
        return BUILTIN_UI_THEMES[themeId] || BUILTIN_UI_THEMES['dark_fantasy'];
    }

    public listThemes(): UITheme[] {
     ...
     */
    public compileScreenSpec(spec: UIScreenSpec): CompiledUIInstance {
        const theme = this.getTheme(spec.theme);
        const targetParent = spec.targetParent || 'StarterGui';

        const rootGui: CompiledUIInstance = {
            className: 'ScreenGui',
    ...
    private compileComponent(comp: UIComponentSpec, theme: UITheme, parentPath: string): CompiledUIInstance {
        ...
        let className = 'Frame';
        if (comp.type === 'Button' || comp.type === 'IconButton') className = 'TextButton';
        else if (comp.type === 'ScreenGui') className = 'ScreenGui';

        const compiled: CompiledUIInstance = {
            className,
            name: comp.id,
            parentPath,
            properties: {
     ...
     */
    public async createScreen(spec: UIScreenSpec): Promise<ExecutionResult> {
        ...
        const rootRecord = await executionPipeline.execute(
            'instance_create',
            {
    ...
    private async instantiateChildren(children: CompiledUIInstance[], parentPath: string): Promise<void> {
     ...
     */
    public critiqueUI(rootPath: string, properties: Record<string, any>): { score: number; passed: boolean; recommendations: string[]; issues: string[] } {
        const issues: string[] = [];
