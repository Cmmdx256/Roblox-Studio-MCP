...
export interface EconomySimulationResult {
    ...
    distribution: Record<string, { count: number; empiricalPercent: number; theoreticalPercent: number }>;
    averageIncomePerMinute: number;
    estimatedTimeToTier: Record<number, { minutesToReach: number; sessionsRequired: number }>;
    economyStabilityScore: number; // 0 - 100
    warnings: string[];
}
...
export class GameplaySimulationEngine {
     ...
     */
    public simulateEconomy(config: EconomySimulationConfig): EconomySimulationResult {
        ...
        const distribution: EconomySimulationResult['distribution'] = {};
        for (const item of config.lootTable) {
        ...
        const timeToTier: Record<number, { minutesToReach: number; sessionsRequired: number }> = {};
