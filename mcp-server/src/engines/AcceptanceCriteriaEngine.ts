...
export class AcceptanceCriteriaEngine {
     ...
     */
    public generateCriteria(intent: StructuredIntent): AcceptanceSuite {
        ...
        for (const req of intent.requirements) {
            ...
            let verType: AcceptanceCriterion['verificationType'] = 'DATAMODEL_INSPECTION';
     ...
     */
    public evaluateSuite(suite: AcceptanceSuite, executedEvidence: ExecutionEvidenceItem[]): AcceptanceSuite {
        let passed = 0;
        let failed = 0;
        let blocked = 0;
        ...
        for (const criterion of suite.criteria) {
            ...
            const ev = executedEvidence.find(e => {
                if (!criterion.targetPath || !e.target) return false;
