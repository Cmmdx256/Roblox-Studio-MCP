...
export class RegressionEngine {
    private testSuite: RegressionTestCase[] = [];
    ...
    private registerDefaultTests(): void {
        this.testSuite.push({
            ...
            checkFn: async () => {
                ...
                if (nodes.length > 0 || sysNodes.length > 0) return true;
                if (commandDispatcher.isStudioConnected()) {
                    ...
                    return Boolean(res && (res as any).success);
                }
    ...
    public registerTest(test: RegressionTestCase): void {
     ...
     */
    public getTargetedTests(modifiedNodeId?: string): RegressionTestCase[] {
        if (!modifiedNodeId) return this.testSuite;
        ...
        const filtered = this.testSuite.filter(t => 
            affectedNames.has(t.system.toLowerCase()) || 
     ...
     */
    public async runRegressionSuite(modifiedNodeId?: string): Promise<RegressionReport> {
