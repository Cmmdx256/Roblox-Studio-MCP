...
export class VisualConstructionEngine {
    ...
    private initializeTemplates(): void {
        const templateList: ComponentTemplate[] = [
            {
                id: 'interactive_door',
    ...
    public listTemplates(): ComponentTemplate[] {
        return Array.from(this.templates.values());
    }

    public getTemplate(id: string): ComponentTemplate | undefined {
     ...
     */
    public async composeComponent(spec: ComponentSpecification): Promise<ExecutionResult> {
     ...
     */
    public async scaffoldHierarchy(): Promise<ExecutionResult> {
        const start = Date.now();
...
`;
    ...
    private generateConstructionScript(templateId: string, name: string, parentPath: string, pos: [number, number, number], attrs: Record<string, any>, includeScript: boolean): string {
        switch (templateId) {
