...
export interface RigPoseSpecification {
    ...
    rigType?: 'R15' | 'R6';
    poses: Record<string, { x?: number; y?: number; z?: number; cframeAnglesDeg?: [number, number, number] }>;
}
...
export interface KeyframeSequenceSpec {
    ...
    keyframes: Array<{
        time: number;
        poses: Record<string, [number, number, number]>; // Joint name -> Euler angles in degrees
    }>;
}
...
export class AnimationAuthoringEngine {
     ...
     */
    public async calibrateToolGrip(spec: ToolGripCalibrationSpec): Promise<ExecutionResult> {
...
`;
     ...
     */
    public async poseRig(spec: RigPoseSpecification): Promise<ExecutionResult> {
        const start = Date.now();
...
`;
     ...
     */
    public async createKeyframeSequence(spec: KeyframeSequenceSpec): Promise<ExecutionResult> {
        const start = Date.now();
