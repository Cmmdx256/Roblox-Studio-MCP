...
export class RecoveryEngine {
     ...
     */
    public async attemptRecovery(errorStr: string, sourceContext?: { scriptPath?: string; sourceCode?: string }): Promise<AutonomousRecoveryResult> {
        ...
        if (!session) {
            return {
                ...
                success: false,
                verification: this.notVerifiableReport(
                    options[0].targetScript || options[0].action || 'recovery',
                    'A live Roblox Studio session is required before a recovery can be applied or verified.'
                ),
                remainingErrors: [errorStr]
            };
        }
    ...
    private classifyError(errorStr: string): ErrorClassification {
    ...
    private determineRootCause(classification: ErrorClassification, errorStr: string): string {
    ...
    private notVerifiableReport(target: string, reason: string): VerificationReport {
        return {
            status: 'NOT_VERIFIABLE',
            verified: false,
            confidence: 0,
