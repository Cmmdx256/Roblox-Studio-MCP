...
export interface AutonomousGamePlan {
    ...
    stages: Array<{ stageNumber: number; name: string; nodeIds: string[] }>;
...
export class AutonomousPlanner {
     ...
     */
    public getNextExecutableNodes(plan: AutonomousGamePlan): FeaturePlanNode[] {
        const completedIds = new Set(
            plan.featureGraph.filter(n => n.status === 'COMPLETED').map(n => n.id)
        );
