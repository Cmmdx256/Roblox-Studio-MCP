...
import { verificationEngine, VerificationReport } from '../verification/VerificationEngine.js';
...
import { studioAvailabilityGuard, RequiredCapability } from '../session/StudioAvailabilityGuard.js';
...
const TERMINAL_NEGATIVE_STATES: ReadonlySet<ExecutionState> = new Set([
 ...
 */
export function assertValidTransition(from: ExecutionState, to: ExecutionState, context = ''): void {
    if (TERMINAL_NEGATIVE_STATES.has(from) && to === 'VERIFIED') {
        throw new Error(
            `[RULE 0 VIOLATION] Illegal state transition: ${from} → VERIFIED. ` +
    ...
    if (!allowed.includes(to)) {
        throw new Error(
            `[ExecutionPipeline] Invalid transition: ${from} → ${to}. ` +
...
export interface PipelineExecutionOptions {
    ...
    autoRecover?: boolean;
    dryRun?: boolean;
    sourceContext?: { scriptPath?: string; sourceCode?: string };
}
...
export class ExecutionPipeline {
    private executionHistory: PipelineExecutionRecord[] = [];

    public async execute(action: string, params: Record<string, any>, options: PipelineExecutionOptions = {}): Promise<PipelineExecutionRecord> {
        ...
        if (!policy.allowed) {
            const blockedReport: VerificationReport = {
                status: 'FAILED',
                verified: false,
                confidence: 0,
                evidence: [],
        ...
        if (options.dryRun) {
            const dryRunReport: VerificationReport = {
        ...
        if (idempotency.isAlreadySatisfied && (idempotency.actionAdvice === 'SKIP' || idempotency.actionAdvice === 'REUSE_EXISTING')) {
            const skippedReport: VerificationReport = {
                status: 'NOT_VERIFIABLE',
                verified: false,
                confidence: 0,
    ...
    public getRecentExecutions(limit = 20): PipelineExecutionRecord[] {
        return this.executionHistory.slice(-limit);
    }

    private requiredCapabilityFor(action: string): RequiredCapability {
        const lower = action.toLowerCase();
        if (lower.includes('playtest')) return 'RUN_PLAYTEST';
        if (lower.includes('screenshot') || lower.includes('screen_capture')) return 'CAPTURE_SCREENSHOT';
        if (lower.includes('input') || lower.includes('keyboard') || lower.includes('mouse')) return 'INJECT_INPUT';
        if (lower.endsWith('_get') || lower.includes('inspect') || lower.includes('search') || lower.includes('output')) return 'READ_DATAMODEL';
