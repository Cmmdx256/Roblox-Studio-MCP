...
import { PerformanceRealityReport, VerificationStatus } from './types.js';

export class PerformanceRealityEngine {
    /**
     * Measure performance metrics from live Studio session.
     */
    public async measurePerformance(): Promise<PerformanceRealityReport> {
        const measuredAt = Date.now();
        const observations: string[] = [];

        let instanceCount: number | 'UNAVAILABLE' = 'UNAVAILABLE';
        let partCount: number | 'UNAVAILABLE' = 'UNAVAILABLE';
        let unanchoredPartCount: number | 'UNAVAILABLE' = 'UNAVAILABLE';
        let scriptCount: number | 'UNAVAILABLE' = 'UNAVAILABLE';
        let uiObjectCount: number | 'UNAVAILABLE' = 'UNAVAILABLE';
...
`
        ...
        const riskLevel: PerformanceRealityReport['riskLevel'] =
            (typeof instanceCount === 'number' && instanceCount > 20000) || runtimeErrors > 5 ? 'HIGH' :
