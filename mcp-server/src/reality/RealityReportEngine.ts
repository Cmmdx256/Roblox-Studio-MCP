    ...
    public formatMarkdownReport(report: RealityReport): string {
        ...
        if (report.performanceReality) {
            ...
            if (pr.memoryIndicators.length > 0) {
                for (const mem of pr.memoryIndicators) lines.push(`  - ℹ️ ${mem}`);
            }
            lines.push('');
        }
        ...
        if (report.gameDesignQA) {
            ...
            for (const obs of dq.observations) {
                ...
                if (obs.suggestedAction) lines.push(`  - *Action*: ${obs.suggestedAction}`);
            }
            lines.push('');
        }
