...
import { CapabilityCompiler, capabilityCompiler } from './CapabilityCompiler.js';
import { CapabilityProbe, capabilityProbe } from './CapabilityProbe.js';
import { CapabilityRouter, capabilityRouter } from './CapabilityRouter.js';
import { providerRegistry } from '../providers/ProviderRegistry.js';
import { CapabilityState, ExecutionResult, FailureCode, ProviderCapability, RiskLevel, SecurityLevel, VerificationMethod } from '../providers/types.js';
...
export class UniversalCapabilityEngine {
    ...
    constructor(
        graph: CapabilityGraph = capabilityGraph,
        compiler: CapabilityCompiler = capabilityCompiler,
        probe: CapabilityProbe = capabilityProbe,
     ...
     */
    public async resolveCapability(intentOrAction: string, context?: Record<string, any>): Promise<CapabilityResolution> {
        ...
        if (toolMap.has(intentOrAction) || (directTool && directTool.state === CapabilityState.AVAILABLE)) {
            return {
                ...
                provider: directTool?.provider as string || 'embedded-plugin',
                confidence: 1.0,
     ...
     */
    public async executeCapability(intentOrAction: string, params: Record<string, any> = {}): Promise<ExecutionResult> {
     ...
     */
    private async executeCompiledPlan(plan: any): Promise<ExecutionResult> {
