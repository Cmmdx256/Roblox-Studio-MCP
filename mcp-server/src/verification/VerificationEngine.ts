...
export interface VerificationReport {
    ...
    verified: boolean; // true when status === 'VERIFIED'
    confidence: number;
...
export class VerificationEngine {
     ...
     */
    public async verifyConditions(conditions: ConditionSpec[], options: { tolerance?: number } = {}): Promise<VerificationReport> {
            ...
            try {
                switch (cond.type) {
                    case 'EXISTENCE': {
                        ...
                        else mismatches.push({ field: `${cond.target}:existence`, expected, actual: exists, reason: `Existence expected ${expected} but was ${exists}` });
                    ...
                    case 'CLASS_NAME': {
                        ...
                        if (matched) passedCount++;
                        else mismatches.push({ field: `${cond.target}:className`, expected: cond.expected, actual: actualClass, reason: `ClassName mismatch` });
                    ...
                    case 'PARENT': {
                        ...
                        if (matched) passedCount++;
                        else mismatches.push({ field: `${cond.target}:parent`, expected: cond.expected, actual: actualParent, reason: `Parent mismatch` });
                    ...
                    case 'PROPERTY': {
                        ...
                        if (matched) passedCount++;
                        else mismatches.push({ field: `${cond.target}.${propName}`, expected: cond.expected, actual: actualProp, reason: `Property value mismatch` });
                    ...
                    case 'ATTRIBUTE': {
                        ...
                        if (matched) passedCount++;
                        else mismatches.push({ field: `${cond.target}#${attrName}`, expected: cond.expected, actual: actualAttr, reason: `Attribute value mismatch` });
                    ...
                    case 'TAG': {
                        ...
                        if (matched) passedCount++;
                        else mismatches.push({ field: `${cond.target}:tag:${tagName}`, expected: cond.expected, actual: hasTag, reason: `CollectionService tag mismatch` });
                    ...
                    case 'SCRIPT_EXACT': {
                        ...
                        if (matched) passedCount++;
                        else mismatches.push({ field: `${cond.target}:source`, expected: cond.expected || cond.pattern, actual: source ? `${source.slice(0, 100)}...` : null, reason: 'Script source did not match pattern' });
     ...
     */
    public async verifyPostconditions(spec: PostconditionSpec, maxRetries = 3, delayMs = 50): Promise<VerificationReport> {
        const conditions: ConditionSpec[] = [];
        if (spec.shouldExist !== undefined) {
            conditions.push({ type: 'EXISTENCE', target: spec.target, expected: spec.shouldExist });
        }
     ...
     */
    public async wrapWithVerification<T>(
        action: string,
        params: Record<string, any>,
        preSpec?: PreconditionSpec | ConditionSpec[],
        postSpec?: PostconditionSpec | ConditionSpec[]
    ): Promise<{ result: T | null; verification: VerificationReport }> {
    ...
    private async queryInstanceExistence(target: string): Promise<boolean> {
        if (!commandDispatcher.isStudioConnected()) return false;
        try {
    ...
    private async queryInstanceMeta(target: string): Promise<{ className?: string; parent?: string } | null> {
        if (!commandDispatcher.isStudioConnected()) return null;
        try {
            const res = await commandDispatcher.executeCommand('instance_get_details', { path: target, target });
            if (res && !res.error) {
    ...
    private async queryProperty(target: string, property: string): Promise<any> {
        if (!commandDispatcher.isStudioConnected()) return null;
        try {
    ...
    private async queryAttribute(target: string, attribute: string): Promise<any> {
        if (!commandDispatcher.isStudioConnected()) return null;
        try {
    ...
    private async queryHasTag(target: string, tag: string): Promise<boolean> {
        if (!commandDispatcher.isStudioConnected()) return false;
        try {
            const res = await commandDispatcher.executeCommand('tag_get_all', { path: target, target });
            if (Array.isArray(res)) return res.includes(tag);
            if (res?.tags && Array.isArray(res.tags)) return res.tags.includes(tag);
        } catch {}
        return false;
    }

    private async queryScriptSource(target: string): Promise<string | null> {
        if (!commandDispatcher.isStudioConnected()) return null;
        try {
    ...
    private compareValues(expected: any, actual: any, tolerance = 0.001): boolean {
