...
import { intentEngine, StructuredIntent } from '../engines/IntentEngine.js';
import { acceptanceCriteriaEngine, AcceptanceSuite } from '../engines/AcceptanceCriteriaEngine.js';
import { changePlanEngine, ChangeOperation, StructuredChangePlan } from '../engines/ChangePlanEngine.js';
import { regressionEngine } from '../engines/RegressionEngine.js';
import { buildHistoryEngine, BuildArtifact } from './BuildHistoryEngine.js';
...
export interface FullOrchestrationResult {
    ...
    stepResults: Array<{ operationId: string; description: string; result: ExecutionResult }>;
}

/** Dispatches a single ChangeOperation to the correct execution backend. Fully capability-driven — no genre coupling. */
async function dispatchOperation(op: ChangeOperation): Promise<ExecutionResult> {
    try {
        switch (op.type) {
            case 'CREATE_SCRIPT': {
...
export class AIOrchestrator {
    ...
    public async orchestrateTask(
        prompt: string,
        mode: OrchestratorOperatingMode = 'AUTONOMOUS'
        ...
        const stepResults: Array<{ operationId: string; description: string; result: ExecutionResult }> = [];
        let verifiedCount = 0;
            ...
            if (succeeded) {
                ...
                if (execResult.status !== 'BLOCKED' && execResult.errors && execResult.errors.length > 0) {
                    ...
                    if (rec.success) {
                        ...
                        projectMemory.recordErrorResolution(
                            rec.classification ?? 'UNKNOWN',
                            execResult.errors[0],
                            rec.rootCause ?? 'Root cause diagnosed',
                            rec.appliedStrategy ?? 'auto_patch'
                        );
        ...
        const evaluatedSuite = acceptanceCriteriaEngine.evaluateSuite(
            acceptanceSuite,
            changePlan.operations.map((op, idx) => {
        ...
        const buildArtifact = buildHistoryEngine.recordBuild(
            intent.summary,
            changedInstances,
            changedScripts,
            evaluatedSuite.passedCount,
            evaluatedSuite.criteria.length,
            undefined,
            { qualityReport }
        );
        ...
        projectMemory.recordDecision(
            prompt,
            `Build outcome for ${intent.domain}: ${buildArtifact.status}; ${verifiedCount}/${changePlan.operations.length} operations independently verified (Build #${buildArtifact.buildNumber}).`,
            `Acceptance Criteria: ${evaluatedSuite.passedCount}/${evaluatedSuite.criteria.length} passed. Regressions: ${regressionReport.hasRegressions ? 'YES' : 'none'}`
        );
        ...
        const overallStatus: FullOrchestrationResult['overallStatus'] =
            buildArtifact.status === 'VERIFIED_COMMIT' ? 'VERIFIED' :
