...
export class ModelingProvider implements IProvider {
  public readonly name = 'modeling-provider';
  public readonly type = ProviderType.MODELING;
  ...
  public async generateModel(
    prompt: string,
    options?: { category?: string; parent?: string; position?: [number, number, number]; scale?: [number, number, number]; anchored?: boolean }
  ...
  public async generateMaterial(prompt: string, baseMaterial = 'SmoothPlastic'): Promise<ExecutionResult> {
    const startTime = Date.now();
  ...
  public async inspectQuality(targetInstancePath: string): Promise<{ passed: boolean; score: number; issues: string[]; recommendations: string[] }> {
    const issues: string[] = [];
