...
import { realityEngine, studioObservationEngine, gameplayStateObserver, gameDesignQAEngine, realityReportEngine, performanceRealityEngine, multiplayerRealityEngine, uiRealityEngine } from './reality/index.js';
...
const platformTools = [
        ...
        handler: async (args) => {
            ...
            if (args.action === 'commit') {
                if (!args.transactionId)
                    throw new Error('transactionId required for commit');
                return await transactionEngine.commitTransaction(args.transactionId);
            }
            if (args.action === 'rollback') {
                if (!args.transactionId)
                    throw new Error('transactionId required for rollback');
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const obs = await observationEngine.observeInstance(args.target, args.cost);
    ...
    },
    {
        ...
        handler: async (args) => {
            const result = await aiOrchestrator.orchestrateTask(args.intent, args.mode || 'AUTONOMOUS');
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const result = { status: 'SUCCESS', constraints };
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const realityReport = await realityEngine.runFullCycle(orchResult.intent, orchResult.changePlan, orchResult.acceptanceSuite, orchResult.stepResults, orchResult.buildArtifact?.buildId || 'build_latest');
