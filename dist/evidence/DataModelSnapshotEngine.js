...
export class DataModelSnapshotEngine {
    ...
    async capture(label = 'snapshot') {
        ...
        try {
            ...
            const rawNodes = response?.result ?? [];
    ...
    diff(snapshotAId, snapshotBId) {
        ...
        if (!a.isRealStudioSnapshot || !b.isRealStudioSnapshot)
            return this.emptyDiff(snapshotAId, snapshotBId);
        const aByPath = new Map(a.nodes.map(n => [n.path, n]));
        const bByPath = new Map(b.nodes.map(n => [n.path, n]));
        ...
        for (const [path, bNode] of bByPath) {
            ...
            for (const property of propertyNames) {
                ...
                if (JSON.stringify(oldValue) !== JSON.stringify(newValue)) {
                    ...
                    if (!modifiedInstances.includes(path))
                        modifiedInstances.push(path);
                }
            }
        ...
        for (const [runtimeId, before] of aByRuntimeId) {
            ...
            if (before.name !== after.name)
                renamedInstances.push({ path: after.path, oldName: before.name, newName: after.name });
            if (before.parentPath !== after.parentPath)
                reparentedInstances.push({ path: after.path, oldParent: before.parentPath ?? '', newParent: after.parentPath ?? '' });
        }
        const result = {
...
//# sourceMappingURL=DataModelSnapshotEngine.js.map