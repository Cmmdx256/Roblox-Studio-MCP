...
const DESIGN_CHECKS = [
    ...
    },
    {
        ...
        evaluate: (spec) => {
            const hasAudio = spec.polish?.feedbackSensoryChecklist?.some(c => typeof c === 'string' && c.toLowerCase().includes('sound'));
            const hasVFX = spec.polish?.feedbackSensoryChecklist?.some(c => typeof c === 'string' && (c.toLowerCase().includes('vfx') || c.toLowerCase().includes('particle')));
            if (!hasAudio && !hasVFX)
                return {
                    type: 'DESIGN_RISK',
                    aspect: 'player_feedback',
    ...
    },
    {
        ...
        evaluate: (spec) => {
            const hasTiers = (spec.progression?.tiers && spec.progression.tiers.length > 0) ||
                spec.coreLoop?.phases?.some((p) => p.name?.includes('upgrade') || p.name?.includes('progress'));
            if (!hasTiers)
                return {
                    type: 'DESIGN_RISK',
                    aspect: 'progression',
    ...
    },
    {
        ...
        evaluate: (spec, intent) => {
            ...
            if (isMultiplayer) {
                const hasTeams = spec.coreLoop?.phases?.some((p) => JSON.stringify(p).toLowerCase().includes('team'));
                if (!hasTeams)
                    return {
                        type: 'DESIGN_RISK',
                        aspect: 'multiplayer_social',
...
export class GameDesignQAEngine {
    ...
    evaluate(spec, intent) {
        ...
        const status = riskCount > 2 ? 'REVIEW_REQUIRED' :
     ...
     */
    evaluateFromPrompt(prompt, intent) {
        try {
...
//# sourceMappingURL=GameDesignQAEngine.js.map