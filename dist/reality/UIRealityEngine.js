...
export class UIRealityEngine {
    ...
    async verifyDevice(screenName, device, autoRepair, maxCycles) {
        let cycles = 0;
    ...
    extractBounds(rootNode, viewportWidth, viewportHeight) {
        if (!rootNode)
            return [];
        const bounds = [];
        const traverse = (node, parentX = 0, parentY = 0, parentW = viewportWidth, parentH = viewportHeight) => {
    ...
    synthesizeUIRepairPatch(rootPath, defects, device) {
        if (defects.length === 0)
            return null;
...
//# sourceMappingURL=UIRealityEngine.js.map