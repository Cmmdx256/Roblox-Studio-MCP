...
export class DesignerBrain {
     ...
     */
    createGameDesignSpec(prompt, themeOverride) {
     ...
     */
    analyzeDesignIntent(theme, genre = 'RPG') {
    ...
    extractTheme(prompt, genre) {
        const lower = prompt.toLowerCase();
        if (lower.includes('fish'))
            return 'Fishing';
        if (lower.includes('medieval') || lower.includes('fantasy'))
            return 'Medieval Fantasy';
        if (lower.includes('sci-fi') || lower.includes('space') || lower.includes('cyberpunk'))
            return 'Sci-Fi';
        if (lower.includes('cartoon') || lower.includes('anime'))
            return 'Stylized Cartoon';
        if (lower.includes('horror') || lower.includes('dark'))
            return 'Dark Atmosphere';
        if (lower.includes('island') || lower.includes('tropical'))
            return 'Tropical Island';
        return `${genre} Experience`;
    }
    generateGameTitle(prompt, genre, theme) {
        const lower = prompt.toLowerCase();
        if (lower.includes('fish'))
            return 'Fishing Simulator: Catch & Trade';
        if (genre === 'Obby')
            return 'Cloud Citadel: Towering Obby';
        if (genre === 'Tycoon')
            return 'Industrial Empire Tycoon';
        if (genre === 'Combat')
            return 'Gladiator Arena Championship';
        return genre === 'Custom' ? `${theme} Experience` : `${theme} ${genre}`;
    }
    deriveSystemsArchitecture(genre, mechanics) {
        const systems = [
            {
                name: 'GameConfig',
                type: 'ModuleScript',
...
//# sourceMappingURL=DesignerBrain.js.map