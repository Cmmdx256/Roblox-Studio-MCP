...
export class GameCreationEngine {
     ...
     */
    async parseSpecification(nlPrompt) {
     ...
     */
    planExecutionOrder(featureGraph) {
        if (!Array.isArray(featureGraph))
            return [];
     ...
     */
    async createGameFromSpec(nlPrompt, progressCallback) {
        ...
        if (progressCallback)
            progressCallback('Project Initialization', 'Running');
        memoryManager.startTask(`task_${Date.now()}`, nlPrompt, spec.featureGraph.map((f) => f.name));
        stepsCompleted.push('Project & Task Initialization');
        // Step 2: Build World & Lighting
        if (progressCallback)
            progressCallback('World & Environment Generation', 'Running');
        ...
        if (progressCallback)
            progressCallback('System Deployment', 'Running');
        stepsCompleted.push('Core Systems & Economy Deployed');
        // Step 4: Verification
        if (progressCallback)
            progressCallback('Verification & QA', 'Completed');
...
//# sourceMappingURL=GameCreationEngine.js.map